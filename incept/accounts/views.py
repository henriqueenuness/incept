    ...
    if request.method == "POST":
        ...
        if nick and nick != user.nick:
            if user.last_nick_change:
                ...
                faltam = (user.last_nick_change + cooldown) - timezone.now()
                if faltam.total_seconds() > 0:
                    dias = faltam.days()
                    segundos = faltam.seconds()
                    horas = segundos // 3600
                    minutos = horas // 60
                    return HttpResponse(f"faltam {dias} dia(s), {horas} hora(s), {minutos} minuto(s) e {segundos} segundo(s) até você poder trocar de nick novamente" )
            if User.objects.filter(nick=nick).exclude(pk=user.pk).exists():
                return HttpResponse("alguém já está usando este nick, tente outro")
            else:
                user.nick = nick
